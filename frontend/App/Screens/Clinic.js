...
const Clinic = () => {
  ...
  const fetchData = async () => {
    try {
      const [tipsResponse, appointmentsResponse] = await Promise.all([
        axios.get(`${CLINIC_BASE_URL}/notes`),
        axios.get(`${CLINIC_BASE_URL}/visits`),
      ]);
  ...
  const goAddVisit = () => {
    navigation.navigate('Addvisits'); // Navigate to Addvisit screen
  };

  const saveNotesData = () => {
    navigation.navigate('Report'); // Navigate to Addvisit screen
  };
  ...
  return (
    ...
    >
      <View style={styles.container}>
        ...
        <TouchableOpacity style={styles.button} onPress={saveNotesData}>
