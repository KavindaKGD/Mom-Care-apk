...
const Reminders = () => {
  ...
  const fetchData = async () => {
    try {
      const response = await axios.get(`${CLINIC_BASE_URL}/visits`);
      setAppointments(response.data); // _id is already a string from backend
    } catch (error) {
  ...
  const updateNotification = async (id, newNotificationValue) => {
    try {
      await axios.patch(`${CLINIC_BASE_URL}/visits/update-notification/${id}`, {
        Notification: newNotificationValue
      });
      setAppointments(prevAppointments =>
        prevAppointments.map(app =>
          app._id === id ? { ...app, Notification: newNotificationValue } : app
        )
      );
    } catch (error) {
  ...
  const updateCustomizationSetting = async (setting) => {
    if (selectedAppointment) {
      ...
      await updateNotification(selectedAppointment._id, newNotification);
  ...
  const deleteAppointment = async (id) => {
    try {
      ...
      await axios.delete(`${CLINIC_BASE_URL}/visits/delete-visit/${id}`);
      setAppointments(prevAppointments =>
      ...
      <View style={styles.appointmentItem}>
        ...
        <View style={styles.appointmentActions}>
          <Switch
            ...
            onValueChange={(newValue) => updateNotification(item._id, newValue ? "MEN" : "")}
...
const styles = StyleSheet.create({
  ...
  appointmentsContainer: { flex: 1, marginBottom: 16 },
  appointmentItem: {
    flexDirection: 'row',
  ...
  appointmentTitle: { fontSize: 18, fontWeight: 'bold', color: '#C41E3A', marginBottom: 4 },
  appointmentDoctor: { fontSize: 16, color: '#333', marginBottom: 4 },
  appointmentDate: { fontSize: 14, color: '#666' },
  appointmentActions: { flexDirection: 'row', alignItems: 'center' },
  deleteButton: {
    marginLeft: 12,
    backgroundColor: '#FAE3E3',
    borderRadius: 15,
  ...
  customizationTextContainer: { flex: 1 },
  customizationTitle: { fontSize: 16, color: '#333', fontWeight: '500' },
  customizationSwitch: { transform: [{ scaleX: 1.1 }, { scaleY: 1.1 }] },
});
